...
ser = serial.Serial('COM3', 115200, timeout=30)  # open serial port
...
# and no parsing is needed: the image is already preprocessed in the MCU
#img = parse_image_rgb565(img, x_size, y_size)

# Now get the results and print them
result_dim = int.from_bytes(ser.read(), byteorder='big')
result = ser.read(result_dim)
print(result.decode("utf-8"))

# Show the image to have feedback on what the camera sees
img = np.frombuffer(img, dtype=np.uint8).reshape((x_size, y_size, 3))
